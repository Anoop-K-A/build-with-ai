...
export default function DashboardPage() {
  ...
  const [analysisErrorKind, setAnalysisErrorKind] = useState("");
  ...
  const getNextAction = () => {
    if (isHighEvap)
      return "HIGH EVAPORATION INDEX DETECTED. Execute deep mulch protocol and increase watering logic instantly.";
    ...
    return [
      {
        ...
        type: "Creation",
        score: 100,
        weather: "Weather: Pending Sync",
        note: isTomato
          ? "Seed Planting: Organic Heirloom Tomato lifecycle initiated."
          : "Botanical Profile securely recorded in Ledger.",
        thumb: null,
      },
    ];
  });
    ...
    async function fetchWeather() {
      ...
      try {
        const res = await fetch(
          `http://localhost:5000/api/weather?lat=${plant.location.lat}&lon=${plant.location.lng}`,
        );
        if (res.ok) {
  ...
  const updateLedgerMemory = (score, diagString) => {
    const newRecord = {
      ...
      score: score,
      weather: weather
        ? `${Math.round(weather.temperature)}°C, ${weather.humidity}% Hum`
        : "Weather: Pending Sync",
      note: diagString,
      thumb: previewUrl,
    };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      updateLedgerMemory(
        result.healthScore,
        result.diagnosis.substring(0, 65) + "...",
      );
    } catch (err) {
      console.warn("Frontend API Catch:", err);
      const message =
        err.message || "Gemini analysis is unavailable right now.";
      setAnalysisError(message);
      setAnalysisErrorKind(
        message.toLowerCase().includes("quota") ? "quota" : "generic",
      );
    } finally {
  ...
  const renderRadial = (score) => {
    ...
    return (
      <div className="relative w-24 h-24 flex outline-none drop-shadow-md">
        ...
        <span className="absolute inset-0 flex items-center justify-center font-black text-2xl text-forest">
          {score}
        </span>
      </div>
    );
  ...
  return (
    <div className="p-8 md:p-12 max-w-7xl mx-auto space-y-10">
      ...
      {thresholdBreach && (
        <div
          className={`w-full p-5 rounded-[1.5rem] flex items-start gap-4 shadow-xl border ${isHighEvap ? "bg-red-50 border-red-500 shadow-red-500/20" : "bg-yellow-50 border-yellow-500 shadow-yellow-500/20"} animate-in slide-in-from-top-10`}
        >
          <div
            className={`p-3 rounded-full ${isHighEvap ? "bg-red-100 text-red-600" : "bg-yellow-100 text-yellow-600"}`}
          >
            <BellRing className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h3
              className={`font-extrabold text-lg ${isHighEvap ? "text-red-700" : "text-yellow-700"}`}
            >
              {isHighEvap
                ? "CRITICAL EVAPORATION RISK DETECTED"
                : "Proactive Notification"}
            </h3>
            <p
              className={`font-medium ${isHighEvap ? "text-red-600" : "text-yellow-700"} mt-1`}
            >
              {getNextAction()}
            </p>
          </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-6 pb-8 border-b border-forest/10">
        <div className="flex items-start gap-6">
          <Link
            to="/collection"
            className="glass-card p-3 rounded-2xl text-forest hover:bg-white hover:scale-105 transition-all hidden md:block"
          >
            <ArrowLeft className="w-6 h-6" />
          </Link>
          <div>
            <div className="flex items-center gap-3 mb-2">
              <h1 className="text-4xl md:text-5xl font-extrabold tracking-tighter text-forest">
                {plant.nickname}
              </h1>
              {weather && (
            ...
            <p className="text-base text-forest/60 font-bold tracking-wide uppercase">
              {plant.species}
            </p>
              ...
              <div className="flex items-center gap-2 mt-5 bg-white/60 backdrop-blur-sm border border-sage/40 text-forest px-4 py-2.5 rounded-[1rem] text-sm font-extrabold w-fit shadow-md relative">
                ...
                <span>
                  {plant.location.name
                    ...
                    : weather?.city
                      ? weather.city
                      : `Lat: ${plant.location.lat.toFixed(2)}, Lng: ${plant.location.lng.toFixed(2)}`}
                </span>
                {lastSync && (
                  <span className="ml-2 pl-3 border-l-2 border-forest/20 text-forest/50 font-bold tracking-wide">
                    SYNCED: {lastSync}
                  </span>
                )}
            ...
            {diagnosisResult && (
              <div
                className={`mt-4 flex items-center gap-3 px-5 py-3 rounded-2xl border transition-all animate-in slide-in-from-left duration-300 ${diagnosisResult.isFallback ? "bg-red-500/10 border-red-500/30 text-red-700" : "bg-forest/10 border-forest/20 text-forest"}`}
              >
                <div
                  className={`w-2 h-2 rounded-full animate-pulse ${diagnosisResult.isFallback ? "bg-red-500" : "bg-forest"}`}
                />
                <span className="text-xs font-black uppercase tracking-tighter">
                  {diagnosisResult.isFallback
                    ? "AI SYNC FAILED: Invalid API Key - Using Local Simulation"
                    : "AI SYNC ACTIVE: Gemini 1.5 Real-Time Analysis"}
                </span>
              </div>
            )}
          </div>
        ...
        <button
          onClick={async () => {
            ...
            if (offlineStr) {
              const arr = JSON.parse(offlineStr);
              localStorage.setItem(
                "rooted_offline_ledger",
                JSON.stringify(arr.filter((p) => p.id !== plantId)),
              );
            }
        ...
        >
          ...
          <span className="hidden sm:inline">
            {confirmDelete ? "Tap again to Confirm" : "Delete Plant"}
          </span>
        </button>
      </div>
            ...
            {!previewUrl ? (
              <label className="relative z-10 flex flex-col items-center justify-center w-full min-h-[350px] border-2 border-dashed border-forest/20 rounded-[2rem] bg-white/40 hover:bg-white/60 cursor-pointer transition-all duration-300 shadow-inner group">
                ...
                <span className="text-lg font-bold text-forest">
                  Upload Leaf Sample or pH Soil Test Array
                </span>
                <span className="text-sm font-medium text-forest/50 mt-2">
                  Analyzed strictly for Organic Interventions
                </span>
                <input
                  type="file"
                  className="hidden"
              ...
              <div className="relative z-10 space-y-6">
                <div className="relative rounded-[2rem] overflow-hidden shadow-xl border-4 border-white/50 bg-forest isolate flex justify-center min-h-[300px]">
                  <img
                    src={previewUrl}
                    alt="Target"
                    className={`w-full max-h-[450px] object-cover transition-all duration-1000 ${loading ? "opacity-70 scale-105 blur-[2px]" : ""}`}
                  />
                  {loading && (
                    <div className="absolute inset-0 z-20 overflow-hidden pointer-events-none">
                      <div
                        className="w-full h-1.5 bg-sage shadow-[0_0_25px_8px_rgba(149,213,178,0.9)] animate-scan"
                  ...
                  {!loading && (
                    <button
                      ...
                      className="absolute top-4 right-4 bg-white/30 backdrop-blur-md text-white font-bold px-4 py-2 rounded-xl"
                    >
                      Clear Matrix
                    </button>
                  )}
                ...
                {!loading && !diagnosisResult && (
                  <button
                    ...
                    className="w-full bg-forest hover:bg-forest/90 text-cream font-extrabold text-lg py-5 rounded-2xl shadow-xl hover:-translate-y-1 transition-all flex justify-center gap-3"
                  >
                ...
                {loading && !diagnosisResult && (
                  <div className="bg-white/60 p-8 rounded-[2rem] border border-white shadow-sm">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-xl font-extrabold text-forest animate-pulse">
                ...
                {analysisError && !loading && !diagnosisResult && (
                  <div className="bg-red-500/10 border border-red-500/30 text-red-700 p-5 rounded-[1.5rem] font-bold space-y-2">
                    <div className="uppercase tracking-wide text-xs font-black">
                    ...
                    {analysisErrorKind === "quota" && (
                      <div className="text-sm font-semibold text-red-600/80">
                        Enable billing or switch to a key/project with remaining
                        Gemini quota.
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gradient-to-b from-white/90 to-sage/10 p-8 rounded-[2rem] border border-white shadow-lg animate-in slide-in-from-bottom duration-500">
                    <div className="flex justify-between items-center mb-6 pb-6 border-b border-forest/10">
                      <h3 className="text-2xl font-extrabold text-forest">
                        Organic Remediation Report
                      </h3>
                      {renderRadial(diagnosisResult.healthScore)}
                    </div>
                    <p className="text-forest font-semibold text-lg leading-relaxed mb-6">
                    ...
                    <ul className="space-y-3 mb-8">
                      {diagnosisResult.recommendations?.map((r, i) => (
                        <li
                          key={i}
                          className="flex gap-4 p-4 rounded-xl bg-white/50 border border-white shadow-sm font-bold text-forest"
                        >
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="glass-card p-5 rounded-2xl bg-white/60">
              <p className="text-[10px] font-black text-forest/40 uppercase tracking-widest mb-1">
                <Clock className="w-3 h-3 inline pb-0.5" /> Active
              </p>
            ...
            <div className="glass-card p-5 rounded-2xl bg-white/60">
              <p className="text-[10px] font-black text-forest/40 uppercase tracking-widest mb-1">
                <Leaf className="w-3 h-3 inline pb-0.5" /> Nutrients
              </p>
              <p className="text-xl font-extrabold text-forest">
                {daysFertilized}d Ago
              </p>
            </div>
          </div>
          ...
          <section className="glass-card p-8 rounded-[2rem] border-white/50 h-full relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10 pb-20 max-h-[800px] overflow-y-auto pr-4 hide-scrollbar">
              {ledgerEntries.map((e, i) => (
                <div
                  key={i}
                  className="relative pl-6 border-l-2 border-forest/20 pb-4 animate-in slide-in-from-right duration-500"
                >
                  <div className="absolute w-4 h-4 bg-white rounded-full -left-[9px] top-0 border-2 border-sage shadow-md" />
                  <div className="text-xs font-black text-forest/40 uppercase mb-2">
                    {e.date} • {e.type} Mode
                  </div>
                  ...
                  <div className="flex flex-col gap-2 mb-3">
                    ...
                    {e.thumb && (
                      <img
                        src={e.thumb}
                        alt="Record"
                        className="w-20 h-20 rounded-xl object-cover shadow-md border border-white mt-1"
                      />
                    )}
                  </div>

                  <p className="text-sm text-forest font-bold leading-relaxed">
                    {e.note}
                  </p>
                </div>
              ))}
